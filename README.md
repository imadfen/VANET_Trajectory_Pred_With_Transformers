...
> Use this section as a map for future modifications.

### Change the input features

**File**: `src/datasets/data.py`

1. Edit `SINDData.feature_names` (line 115) — list of column names to use.
2. Edit `keep_cols` in `sort_clean_data()` (line 201) to include the same columns.
3. The model automatically picks up `feat_dim = len(data.feature_names)` —
   no architecture changes needed.

### Add a new feature from the CSV (e.g. `VehicleLength`)

Same as above — just append the column name to `feature_names` and `keep_cols`.

### Change sequence length / window size

**CLI**: `--data_chunk_len N`  
**File**: `src/options.py` line 124 (default).

### Change masking strategy (e.g. mask only position features)

**CLI**: `--exclude_feats 2,3,4,5,6,7,8` (indices into `feature_names`)  
This keeps Speed, Acceleration, … always visible and only masks X, Y.

### Switch to true imputation loss (only masked positions)

**File**: `src/transformer_model/model.py` lines 381–384

Uncomment:
```python
target_masks = target_masks * padding_masks.unsqueeze(-1)
```
And pass `target_masks` (instead of `padding_masks.unsqueeze(-1)`) to `self.loss_module`.

### Add a proper categorical embedding for LaneID

1. Keep LaneID as integer (factorize is already done).
2. In `encoder.py` `TSTransformerEncoder.__init__`, add:
   ```python
   self.lane_embedding = nn.Embedding(num_lanes, lane_embed_dim)
   ```
3. In `forward()`, split `X` along the feature dim, embed LaneID separately, then
   concatenate before `project_inp`.

### Change the Transformer internal normalisation

**CLI**: `--normalization_layer LayerNorm`  
Use `LayerNorm` if batch size is very small (< 8), otherwise `BatchNorm` is stable and faster.

### Add a downstream classification head

After pre-training, load `model_best.pth`, freeze the encoder, and add a classification
head on top of the embeddings returned by `TSTransformerEncoder.forward()`:

```python
embeddings, embeddings_original = encoder(X, padding_masks)[1]
# embeddings: (B, seq_len, embedding_dim)
# pool across time, then classify:
pooled = embeddings.mean(dim=1)   # (B, embedding_dim)
logits = classifier_head(pooled)
```

### Add a new dataset class

1. Create a new class inheriting `BaseData` in `src/datasets/data.py`.
2. Implement `load_data()` and populate `self.all_chunks`, `self.all_IDs`,
   `self.feature_names`, `self.max_seq_len`.
3. Register it: `data_factory["mydata"] = MyDataClass`.
4. Use it: `--data_class mydata`.

### Tune a single hyperparameter manually
